    ...
    def wrap_modules(
        ...
        for module_index, module in enumerate(modules_generator):
            ...
            if module_index % self.group_size >= self.group_size - self.num_in_group:
                ...
                submodule = (
                    submodule_accessor(module) if submodule_accessor else module
                )
                ...
                logger.info(
                    f"[offloader] Offloading module {module_index}: "
                    f"submodule={type(submodule).__name__}, "
                    f"params={whitelist_param_names}, "
                    f"memory_allocated={torch.cuda.memory_allocated() / 1e9:.2f}GB"
                )
...
class _ModuleOffloader:
    ...
    def __init__(
        ...
        self._device_tensors: Optional[Dict[str, torch.Tensor]] = None
        self._load_event: Optional[torch.cuda.Event] = None
