...
def create_fast_balanced_ubatch_slices(
    ...
    if num_requests <= 32:
        return _create_simple_consecutive_ubatch_slices_fast(
            workload_info, num_ubatches
        )

    # For large batches, use fast greedy splitting
    return _create_fast_greedy_ubatch_slices(
...
def _create_simple_consecutive_ubatch_slices_fast(
    ...
    for i in range(num_ubatches):
        ...
        token_start = (
            int(torch.sum(workload_info.query_lens[:request_start]))
            if request_start > 0
            else 0
        )
...
def _create_fast_greedy_ubatch_slices(
    ...
    for req_indices in ubatch_request_lists:
        ...
        if len(ranges) == 1:
            ...
            token_start = (
                int(torch.sum(workload_info.query_lens[:start])) if start > 0 else 0
            )
