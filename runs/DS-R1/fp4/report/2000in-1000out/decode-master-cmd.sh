NVIDIA_GDRCOPY=1 \
  ...
  --kv_cache_memory_bytes 119019622533 \
  --data-parallel-size-local 4 \
  --stream-interval 50 \
  --max-num-seqs 1220 \
  --max-num-batched-tokens 1220 \
  --cudagraph-capture-sizes 128 512 1220 \
  --kv-transfer-config '{"kv_connector":"NixlConnector","kv_role":"kv_both", "kv_load_failure_policy":"fail"}' --data-parallel-address 10.244.2.151 --data-parallel-size 8 2>&1 | tee decode-master-pd.log
